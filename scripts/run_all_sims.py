...
#   A = stochastic or compute-heavy study with cached --data-only / --plots-only
#   B = deterministic uncached generator; --plots-only recomputes before rendering
# ---------------------------------------------------------------------------
...
REGISTRY = [
    ...
    ("ch03_theory", "ch03_theory/sims/engine_value_polytope.py", "B"),
    ("ch03_theory", "ch03_theory/sims/engine_value_learning.py", "A"),
    ("ch03_theory", "ch03_theory/sims/engine_hybrid_search.py", "B"),
    # ch03a_bm
    ...
    ("ch03b", "ch03b_deeprl_practice/sims/engine_bellman_error.py", "B"),
    # ch04
    ...
    ("ch05", "ch05_econ_models/sims/engine_estimation.py", "A"),
    # ch06_macro
    ...
    ("ch09", "ch09_rlhf/sims/engine_preferences.py", "B"),
    # ch10
    ("ch10", "ch10_causal/sims/confounded_ope.py", "A"),
    ("ch10", "ch10_causal/sims/identification_dags.py", "B"),
    ("ch10", "ch10_causal/sims/engine_confounding.py", "A"),
    ...
    ("ch10b", "ch10b_rl_for_ci/sims/engine_ope.py", "A"),
    ...
    ("ch11", "ch11_dist_robust_constrained/sims/engine_occupancy_kl.py", "B"),
    ...
    ("ch12", "ch12_world_models/sims/engine_model_learning.py", "A"),
...
def main():
    ...
    # Build common flags. Component selectors are passed only to scripts that expose
    # add_component_args; deterministic and monolithic scripts still run normally.
    common_flags = []
    if args.data_only:
        common_flags.append("--data-only")
    if args.plots_only:
        common_flags.append("--plots-only")
    ...
    shown_flags = list(common_flags)
    if args.algo:
        for algo in args.algo:
            shown_flags.extend(["--algo", algo])
    print(
        f"Running {len(scripts)} scripts"
        + (f" with flags: {' '.join(shown_flags)}" if shown_flags else "")
        + "\n"
    )
    ...
    for ch, path, cat in scripts:
        ...
        if args.algo:
            ...
            if "add_component_args" in source:
                for algo in args.algo:
                    flags.extend(["--algo", algo])
