...
def solve_oracle_dp(r, K, p, c, sigma, gamma, n_s=60, n_h=31, tol=1e-7, max_iter=400):
    ...
    for i, s in enumerate(s_grid):
        ...
        g[i] = best_h
    return dict(
        s_grid=s_grid, h_grid=h_grid, V=V, g=g,
        h_msy=h_msy, h_max=h_max, s_max=s_max,
...
if __name__ == "__main__":
    ...
    print(f"  V(0)={oracle['V'][0]:.3f}  V(K/2)={np.interp(K/2, oracle['s_grid'], oracle['V']):.3f}  V(K)={oracle['V'][-1]:.3f}")
    print(f"  g(K/2)={np.interp(K/2, oracle['s_grid'], oracle['g']):.3f} (target MSY={r*K/4:.3f})")
    ...
    print(f"  s_0={s_history[0]:.3f}  s_50={s_history[50]:.3f}  s_100={s_history[-1]:.3f}")
