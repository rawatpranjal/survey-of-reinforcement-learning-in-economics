...
OUTPUT_PATH = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "dyna_maze_layout.png"
)
...
def main():
    # Runner-interface flags: diagram-only script, --data-only is a no-op
    # exit and --plots-only runs normally.
    import argparse
    ...
    if args.data_only:
        print("draw_maze_layout is diagram-only; --data-only is a no-op.")
        return
    fig, (ax1, ax2) = plt.subplots(1, 2, figsize=(11, 4))
    draw_phase(
        ax1,
        BlockingMaze.PHASE1_WALL_COLS,
        "Phase 1 ($t \\leq 1000$): opening at column 8",
    )
    draw_phase(
        ax2, BlockingMaze.PHASE2_WALL_COLS, "Phase 2 ($t > 1000$): opening at column 0"
    )
    fig.suptitle(
        "Sutton blocking maze: $6 \\times 9$ gridworld with phase-switchable wall",
        fontsize=12,
    )
    fig.tight_layout()
