...
\subsubsection{Least-Squares Temporal Difference Learning (1996)}

\citet{BradtkeBarto1996} replaced the stochastic-approximation solver of TD(0) with a moment estimator. TD(0) with linear features converges to the parameter solving $A\theta = b$, where $A$ and $b$ are expectations of feature products under the stationary distribution; least-squares TD estimates those two expectations from the sample and solves the linear system directly. The estimator has no step size, so it cannot be slowed or destabilized by a poor choice of one, and it extracts more from each transition, at a cost quadratic rather than linear in the number of features. The convergence argument treats the system as an instrumental-variables regression, with the current features instrumenting for a regressor that is measured with error. The population target is the same projected fixed point that TD(0) approaches, so the two algorithms differ in solver rather than in destination (Section~\ref{sec:bellman_estimation}).
...
\subsubsection{Least-Squares Policy Iteration (2003)}

\citet{LagoudakisParr2003} carried least-squares TD to action values and embedded it in policy iteration. Their LSTDQ solves the same linear system on the chain of state-action pairs, and because forming the system requires only the action the target policy would take at each recorded successor state, one fixed batch of transitions can evaluate any policy. LSPI alternates this evaluation with greedy improvement until the policy stops changing. The resulting algorithm is off-line and off-policy, which made it a standard batch method before the deep reinforcement learning literature took up the same problem, and it carries a stability guarantee bounding the eventual policy's suboptimality by a constant multiple of the worst per-iteration value-approximation error (Section~\ref{sec:bellman_estimation}).
