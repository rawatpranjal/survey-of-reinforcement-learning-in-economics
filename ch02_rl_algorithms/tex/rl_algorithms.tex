...
% Running example introduction, then two sections: Classical Synthesis (Sutton-Baird),
% Deep Learning Era (Mnih-AlphaGo)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\subsection{A Running Example: Bus Engine Replacement}
\label{engine:model}
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

The algorithms of this chapter, and much of the theory in the chapters that follow, are computed on one fixed model. A maintenance manager watches a bus engine that sits at low or high mileage, so $\mathcal{S} = \{\text{low}, \text{high}\}$, and each period either keeps it or replaces it, so $\mathcal{A} = \{\text{keep}, \text{replace}\}$. Keeping a low-mileage engine yields output $1$ and wears the engine to high mileage with probability $\tfrac{1}{2}$. Keeping a high-mileage engine yields output $0.2$ and leaves it there. Replacement costs $0.5$ from either state and returns the engine to low mileage with certainty, and the discount factor is $\gamma = 0.9$. The model is a two-state instance of the bus engine replacement problem of \citet{Rust1987}, with per-period output standing in for operating profit net of the maintenance cost $\theta_1$ and the replacement charge standing in for the fixed cost $RC$. Figure~\ref{fig:engine_digraph} draws the process.
...
\end{tikzpicture}
\caption{The running example as a labelled directed graph. Each arc carries its action and its transition probability, and, where the action is first named from that state, its reward. Blue arcs are the keep action and orange arcs the replace action.}
\label{fig:engine_digraph}
\end{figure}

The instance is small enough that everything can be checked by hand. The optimal policy keeps at low mileage and replaces at high mileage, with $V^\star = (5.3448,\ 4.3103)$ and action-value gaps $1.0345$ at the low state and $0.2310$ at the high state. Appendix~\ref{prelim:mdp} computes every step of this solution, narrating the same instance as a machine in good or worn condition, and the shared simulation module pins each of these values with exact-arithmetic oracle tests. These values recur through the book. When a chapter introduces a method, this model is the first thing the method runs on, and the estimate it produces is compared against the same $V^\star$.

Table~\ref{tab:engine_algorithms} previews the chapter by running its eight algorithm families on the model. Every family recovers the optimal values to within a tenth of a unit, though the rows are not a race, since the families consume different data budgets and are scored in three different modes. The two policy-gradient rows sit a visible step below $V^\star$ because their final policies remain stochastic, and a sigmoid policy attains the deterministic optimum only in the limit of infinite logits. What separates the families is the object they estimate along the way, a value function for a fixed policy, an action-value function from exploratory data, or the policy itself, and the sections below take them up in roughly that order.

\input{../ch02_rl_algorithms/sims/engine_algorithms.tex}
...
When $P(s'|s,a)$ and $r(s,a)$ are unknown, the obvious approach is to use Monte Carlo (MC) to approximate them.\footnote{The Markov decision process itself, the tuple $(\mathcal{S}, \mathcal{A}, P, r, \gamma)$ together with policies, returns, and the value and action-value functions, is defined in Appendix~\ref{prelim:mdp}, which also fixes the notation used from here on.} These methods estimate value functions from sampled \textit{episodes} $(s_0, a_0, r_1, s_1, a_1, r_2, \ldots, s_T)$. The realized \textit{return} from state $s_t$ is
\begin{equation}
...
Standard gradient descent treats all parameter directions equally, but policy parameters define probability distributions whose natural geometry is not Euclidean. \citet{Kakade2001} introduced the natural policy gradient, which measures progress in distribution space rather than parameter space. The update uses the Fisher information matrix $F(\theta)$.\footnote{Appendix~\ref{prelim:quadratic_forms} records its score outer-product form, local KL geometry, and conditioning.}
\begin{equation}
...
With feature matrix $\Phi \in \mathbb{R}^{|\mathcal{S}| \times d}$ (rows $\phi(s)^\top$) and per-action weight vectors $\theta_a \in \mathbb{R}^d$, each FQI regression step for action $a$ reduces to the normal equations.\footnote{Appendix~\ref{prelim:span_rank} interprets the columns of $\Phi$ as representable directions. Appendix~\ref{prelim:least_squares} derives the normal equations from orthogonality of the regression residual.}
\begin{equation}
...
PPO outperformed A2C, TRPO, and the cross-entropy method on continuous control benchmarks and achieved the highest average reward on 30 of 49 Atari games among the methods tested. PPO became the default policy optimization algorithm for large-scale RL applications.
...
\caption{Architecture comparison of three core algorithm families: value-based, policy gradient, and actor-critic. (a) DQN maps states to Q-values for all actions, selecting the argmax. (b) REINFORCE maps states to a probability distribution over actions, then samples. (c) Actor-Critic maintains separate policy and value networks; the critic's TD error $\delta_t$ feeds back into both during training, yielding lower-variance policy updates than REINFORCE's Monte-Carlo return. Each panel includes the environment feedback loop in which the agent's action produces a reward and next state.}
...
The \emph{control-as-inference} framework \citep{Todorov2006, Kappen2011, Ziebart2010, Levine2018} recasts the preceding algorithms as instances of probabilistic inference (computation, not statistical inference) in a single graphical model.\footnote{The framework is a mathematical language, much like random utility in econometrics, that reveals structural connections rather than deriving algorithms from first principles.} The benefit is that every advance in approximate inference (variational methods, message passing, amortized inference) becomes a candidate RL algorithm, and the forward problem (finding the optimal policy given rewards) and the inverse problem (recovering rewards from observed behavior) become two queries in the same model. The construction introduces a binary ``optimality'' variable $\mathcal{O}_t \in \{0,1\}$ at each time step, appended to the standard state-action-dynamics chain (Figure~\ref{fig:control_inference_dag}). The distribution over this variable is defined as
\begin{equation}
...
Exact inference in this graphical model under stochastic dynamics produces risk-seeking policies. The backup becomes $Q(s,a) = r(s,a) + \tau \log \mathbb{E}_{s'}[\exp(V(s')/\tau)]$, which overweights unlikely favorable transitions because the agent implicitly assumes it can influence the dynamics.\footnote{Under exact inference, the posterior dynamics $P(s'|s,a,\mathcal{O}_{1:T}=1)$ differ from the true dynamics $P(s'|s,a)$, biasing the inferred transitions toward favorable outcomes. The log-expectation-of-exponentials exceeds the expectation by Jensen's inequality (Theorem~\ref{thm:prelim_jensen}). This optimism is an artifact of exact inference in the model, not a feature.} The framework corrects this by applying structured variational inference, restricting the variational family to distributions that match the true dynamics $P(s'|s,a)$. This yields the soft Bellman equations
\begin{align}
