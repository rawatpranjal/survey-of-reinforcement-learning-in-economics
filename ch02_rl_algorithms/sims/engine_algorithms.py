# Eight algorithm families on the Engine Replacement MDP
...
def generate_outputs(data):
    """Generate the algorithm table and the supporting Engine tables."""
    ...
    with open(tex_path, "w") as f:
        ...
        f.write(
            "\\caption{Eight algorithm families on the Engine Replacement MDP, "
            f"{SHARED_CONFIG['n_seeds']} seeds each, ranked by mean sup-norm distance "
            f"from $V^\\star = ({V_star[LOW]:.4f},\\ {V_star[HIGH]:.4f})$. Standard "
            "errors across seeds in parentheses. Rows carry different data budgets and "
            "three scoring modes. Prediction rows "
    ...
    dictionary_path = os.path.join(OUTPUT_DIR, "engine_dictionary.tex")
    with open(dictionary_path, "w") as f:
        f.write("\\begin{table}[h]\n\\centering\n")
        f.write(
            "\\caption{The Engine Replacement MDP in the vocabularies of reinforcement "
            "learning and dynamic economic models.}\n"
        )
        f.write("\\label{tab:engine_dictionary}\n")
        f.write("\\begin{tabular}{@{}lll@{}}\n")
        f.write(
            "\\hline\nreinforcement learning & object & economic model \\\\\n\\hline\n"
        )
        rows = [
            ("state", "mileage grade $s$", "observed condition $x$"),
            ("action", "keep or replace $a$", "discrete choice $d$"),
            ("transition", "$P(s'\\mid s,a)$", "controlled law of motion"),
            ("reward", "$r(s,a)$", "flow profit net of maintenance"),
            ("policy", "$\\pi(a\\mid s)$", "replacement rule"),
            ("action value", "$Q^\\pi(s,a)$", "choice-specific value"),
            ("value", "$V^\\pi(s)$", "pre-choice value"),
        ]
        for obj, rl_term, econ_term in rows:
            f.write(f"{rl_term} & {obj} & {econ_term} \\\\\n")
        f.write("\\hline\n\\end{tabular}\n\\end{table}\n")
    print(f"  Table saved: {dictionary_path}")

    appearances = [
        ("Theory", "value and policy geometry", "engine:value_space"),
        ("Deep RL practice", "Bellman and value error", "engine:ch03b"),
        ("Structural estimation", "parameter recovery", "engine:ch05"),
        ("Offline RL", "coverage and extrapolation", "engine:ch08"),
        ("Human feedback", "reward equivalence", "engine:ch09"),
        ("Causal RL", "unobserved confounding", "engine:ch10"),
        ("OPE and treatment effects", "importance weights", "engine:ch10b"),
        ("Robust and constrained RL", "budget duality", "engine:ch11"),
        ("World models", "transition learning", "engine:ch12"),
    ]
    recap_path = os.path.join(OUTPUT_DIR, "engine_recap.tex")
    with open(recap_path, "w") as f:
        f.write("\\begin{table}[h]\n\\centering\n")
        f.write(
            "\\caption{What the Engine Replacement MDP establishes across the survey.}\n"
        )
        f.write("\\label{tab:engine_recap}\n")
        f.write("\\begin{tabular}{p{0.30\\textwidth}p{0.55\\textwidth}}\n")
        f.write("\\hline\nchapter topic & role of the common model \\\\\n\\hline\n")
        for chapter, obj, label in appearances:
            f.write(f"{chapter} & {obj}, Section~\\ref{{{label}}} \\\\\n")
        f.write("\\hline\n\\end{tabular}\n\\end{table}\n")
    print(f"  Table saved: {recap_path}")


def main():
    parser = argparse.ArgumentParser(
        description="Eight algorithms on the Engine Replacement MDP"
    )
    add_component_args(parser)
    args = parser.parse_args()
    force = parse_force_set(args)
    print("=" * 70)
    print("EIGHT ALGORITHM FAMILIES ON THE ENGINE REPLACEMENT MDP")
    print("=" * 70)
    print()
    if args.plots_only:
