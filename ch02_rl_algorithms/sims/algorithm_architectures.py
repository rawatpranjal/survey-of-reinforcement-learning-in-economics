...
def draw_rect_node(ax, xy, size, label, color, alpha=0.15, fontsize=LABEL_FONT):
...
def draw_edge(
    ...
    if p1_shape == "circle":
        start = _circle_edge_point(
            p1, p2, r=p1_size if isinstance(p1_size, (int, float)) else p1_size[0]
        )
    else:
        start = _rect_edge_point(p1, p1_size, p2)

    if p2_shape == "circle":
        end = _circle_edge_point(
            p2, p1, r=p2_size if isinstance(p2_size, (int, float)) else p2_size[0]
        )
    else:
...
def draw_dqn(ax):
    ...
    # Label the feedback
    ax.text(
        4.3, 0.35, r"$a_t$", fontsize=9, color=COLORS["gray"], ha="center", va="center"
    )
    ax.text(
        0.65,
        0.35,
...
def draw_actor_critic(ax):
    ...
    ax.text(
        4.6, 0.5, r"$a_t$", fontsize=9, color=COLORS["gray"], ha="center", va="center"
    )
    ax.text(
        0.4,
        0.5,
    ...
    ax.text(
        nodes["td"]["xy"][0],
        -0.75,
        r"$\delta_t = r_{t+1} + \gamma V_w(s_{t+1}) - V_w(s_t)$",
        ha="center",
        va="top",
        fontsize=9,
        color=col,
...
def generate_outputs():
    ...
    fig.savefig(
        outpath, dpi=300, bbox_inches="tight", facecolor="white", edgecolor="none"
    )
    plt.close(fig)
