...
\begin{table}[htbp]
\centering
\caption[Multi-agent Q-learning algorithms: convergence and information requirements]{\textbf{Multi-agent Q-learning algorithms: convergence and information requirements.}}
...
Two canonical games from industrial organization serve as benchmarks. In Cournot duopoly, two firms choose quantities $q_i \in \{0, 1, \ldots, 9\}$ with inverse demand $P(Q) = 10 - Q$ and marginal cost $c = 1$; the unique Nash equilibrium is $q^* = 3$ with profit $\pi^* = 9$. In Bertrand duopoly with differentiated products, two firms choose prices $p_i \in \{0, 1, \ldots, 9\}$ with demand $d_i = 10 - 2p_i + p_j$ and marginal cost $c = 1$; the continuous Nash equilibrium is $p^* \approx 4.33$, which discretizes to $p^* = 4$ with profit $\pi^* = 18$. Both games have unique Nash equilibria in pure strategies.\footnote{Each configuration runs for 50,000 iterations across 20 seeds.} Three algorithms compete. They are independent Q-learning (IQL), Nash-Q, and WoLF-PHC.

\begin{table}[htbp]
\centering
\caption[Convergence to Nash equilibrium in Cournot and Bertrand duopoly]{\textbf{Convergence to Nash equilibrium in Cournot and Bertrand duopoly.}}
...
Table~\ref{tab:cournot_bertrand} reports the results. All three algorithms converge to Nash in both games within the first 5,000 iterations. IQL, which lacks any game-theoretic computation, matches the game-aware methods in these well-structured games with unique pure-strategy equilibria. The advantage of Nash-Q and WoLF-PHC emerges in games requiring mixed strategies, where IQL's deterministic policy limit prevents convergence.
...
\subsection{Neural Extensions and Poker}

Tabular CFR stores regrets at every information set, infeasible when $|\mathcal{I}| > 10^{14}$. \citet{Brown2019} approximate cumulative regrets with a neural network trained on sampled (information set, iteration, regret) tuples, weighting by iteration index to prioritize recent estimates; the current strategy derives from regret matching on the network's outputs. NFSP \citep{heinrich2016deep} takes a different approach, combining fictitious play\footnote{Fictitious play \citep{Brown1951} is a classical learning rule where each player best-responds to the empirical distribution of opponents' past actions.} with deep Q-learning. Each player maintains a best-response network $Q_\theta$ and an average strategy network $\bar{\pi}_\phi$ trained via supervised learning.

These methods scale CFR to large imperfect-information games. Deep CFR attained exploitability\footnote{Exploitability measures how far a strategy is from Nash equilibrium, defined as the maximum expected gain an adversary could achieve by best-responding. In poker, it is measured in milli-big-blinds per game (mbb/g).} of 37 mbb/g in heads-up flop hold'em \citep{Brown2019}. Libratus defeated top professionals in heads-up no-limit hold'em using nested subgame solving with CFR \citep{brown2017superhuman}. Pluribus extended to six-player no-limit hold'em \citep{brown2019superhuman}.
...
\begin{table}[htbp]
\centering
\caption[CFR equilibrium vs.\ theory: $\pi$-sweep at $\delta = 0.5$]{\textbf{CFR equilibrium vs.\ theory: $\pi$-sweep at $\delta = 0.5$.}}
\label{tab:coase}
