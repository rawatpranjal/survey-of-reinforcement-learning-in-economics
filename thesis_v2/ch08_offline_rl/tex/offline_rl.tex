...
\subsubsection{Alternative Pessimism Mechanisms}

Other approaches achieve pessimism through different mechanisms. Implicit Q-Learning \citep[IQL,][]{Kostrikov2022} avoids querying unseen actions entirely by learning the value function through expectile regression, approximating the in-sample maximum without evaluating $Q$ at out-of-distribution actions.\footnote{IQL uses an asymmetric squared loss $L_2^\tau(u) = |\tau - \mathbbm{1}\{u < 0\}| \cdot u^2$ with expectile parameter $\tau \in (0.5, 1)$. As $\tau \to 1$, the expectile converges to the in-sample maximum; $\tau = 0.7$ is typical.} Batch-Constrained Q-Learning \citep[BCQ,][]{Fujimoto2019} constrains the policy to actions with high likelihood under the learned behavioral distribution, addressing distributional shift at the policy level rather than the value function level. BCQ's performance is bounded by the quality of actions in the dataset; if the behavioral policy never takes the optimal action at some state, BCQ cannot discover it regardless of sample size.
...
\begin{table}[t]
\centering
\caption[Policy value for each offline RL method, expressed as mean return and percentage of the DP optimal]{\textbf{Policy value for each offline RL method, expressed as mean return and percentage of the DP optimal.} Standard errors computed over 20 seeds.}
...
The learned $r_\theta$ then serves as a proxy objective for policy optimization. The canonical pipeline \citep{ouyang2022training} proceeds in three stages. The first is supervised fine-tuning (SFT) on demonstrations to produce a reference policy $\pi^{SFT}$, the second is reward model training from preferences, and the third is PPO fine-tuning of $\pi_\phi$ against the frozen $r_\theta$ with a KL-divergence penalty preventing drift into regions where $r_\theta$ is unreliable. The objective is
\begin{equation}
...
\begin{table}[htbp]
\centering
\caption[Diagnostics at $K = 5{,}000$ (single seed): policy agreement with $\pi^*$, value-function correlation, and mean accepted wage and amenity for each method]{\textbf{Diagnostics at $K = 5{,}000$ (single seed): policy agreement with $\pi^*$, value-function correlation, and mean accepted wage and amenity for each method.}}
