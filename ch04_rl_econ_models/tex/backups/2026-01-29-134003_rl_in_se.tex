...
\end{equation}
where $u(s,a;\theta) = z(s,a)^\top \theta$ is the deterministic component of per-period utility, $\theta \in \mathbb{R}^d$ collects the structural parameters, and $\varepsilon_t(a)$ is an idiosyncratic shock, typically drawn i.i.d.\ from a Type I extreme value distribution. The econometrician observes panel data $\{(s_{it}, a_{it})\}_{i=1,\ldots,n}^{t=1,\ldots,T}$ and seeks to recover $\theta$.

The classical approach of \citet{Rust1987} requires repeated solution of a fixed-point problem (nested fixed-point, or NFXP), while the conditional choice probability (CCP) method of \citet{HotzMiller1993} avoids this by exploiting an invertibility result linking choice probabilities to value function differences. Both methods, however, face severe computational challenges when the state space is continuous or high-dimensional: NFXP requires discretization and repeated Bellman iteration, while CCP requires nonparametric estimation of transition densities.

Two recent papers bring RL algorithms directly into the DDC estimation pipeline, addressing these computational bottlenecks.
...
The first is the linear semi-gradient method. Approximate $h(a,s) \approx \phi(a,s)^\top w$ where $\phi: \mathcal{A} \times \mathcal{S} \to \mathbb{R}^p$ is a vector of basis functions and $w \in \mathbb{R}^p$ are the weights. The TD(0) fixed-point equation is
\begin{equation}
...
With $\hat{h}$ and $\hat{g}$ in hand, structural parameters are recovered by pseudo-maximum likelihood estimation (PMLE). For continuous state spaces, \citet{AdusumilliEckardt2022} derive a locally robust correction to the PMLE criterion that accounts for the nonparametric first-stage estimation of value terms, restoring $\sqrt{n}$-convergence of $\hat{\theta}$.

\begin{theorem}[\citet{AdusumilliEckardt2022}, Theorem 1]
Under regularity conditions, the linear semi-gradient estimator $\hat{h}$ satisfies $\|\hat{h} - h\|_2 = O_P(n^{-1/2}(T-1)^{-1/2})$, where $\|\cdot\|_2$ denotes the $L^2(P)$ norm.
\end{theorem}
...
The estimation framework is indirect inference. Define a vector of data moments $\mathbf{M}_d$ computed from the observed panel. For candidate parameter $\theta$, simulate the model under $\theta$ to produce simulated moments $\mathbf{M}_s(\theta)$. The estimator minimizes the quadratic distance:
\begin{equation}
...
\citet{AskerEtAl2020} develop a computational framework for analyzing dynamic procurement auctions with serially correlated asymmetric information, building on the Experience-Based Equilibrium (EBE) concept of \citet{FershtmanPakes2012}.\footnote{\citet{FershtmanPakes2012} introduced the EBE concept for dynamic games with asymmetric information. Their approach computes equilibria by simulating industry trajectories and updating strategies toward best responses, evaluating values only on recurrently visited states rather than the full state space. While the EBE computation uses iterative value estimation, it does not employ a standard RL algorithm with online learning updates. We therefore discuss Fershtman and Pakes as a conceptual precursor rather than an RL application proper.}
...
\end{equation}
where $\alpha > 0$ is the learning rate. The equilibrium computation iterates: firms simultaneously update their Q-functions based on simulated play against each other's current strategies. Strategies are derived from Q-values using an $\varepsilon$-greedy rule or Boltzmann exploration.
...
The equilibrium computation follows the Pakes-McGuire iterative scheme: simulate long industry histories, update each firm's value function via (\ref{eq:hollenbeck_td}), re-derive best-response strategies from updated values, and repeat. The RL component replaces the exact Bellman iteration step that would be infeasible over the full state space, focusing computational effort on recurrently visited states.
...
Two related papers merit brief mention. \citet{LomysMagnolfi2024} develop structural estimation methods for strategic settings where agents use reinforcement learning rather than playing a fixed equilibrium. They impose an ``asymptotic no-regret'' condition as a minimal rationality requirement and derive identification results for payoff parameters, but their estimation method is standard econometrics rather than RL.\footnote{\citet{LomysMagnolfi2024} provide an important conceptual contribution: they bridge the gap between the RL-based behavioral models (where agents learn) and the econometric problem (where the researcher estimates structural parameters from data generated by learning agents). Their work complements the papers reviewed in this section by addressing the statistical foundations.} \citet{Covarrubias2022} uses deep RL to study oligopolistic pricing in a New Keynesian framework, representing firms' pricing policies as neural networks $\pi_\phi(a|s)$. The method uncovers multiple equilibria ranging from competitive to collusive pricing, though the training procedure's relationship to standard RL algorithms is less explicit than in the other papers reviewed here.
