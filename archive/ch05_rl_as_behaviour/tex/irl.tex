...
Inverse Reinforcement Learning (IRL) addresses a fundamental problem: given observed agent behavior, how can we recover the reward function that motivated it? Formally, given an MDP $\mathcal{M} \setminus r = (\mathcal{S}, \mathcal{A}, P, \gamma)$ without the reward function, and demonstrations $\mathcal{D} = \{\tau_1, \tau_2, ..., \tau_m\}$ where each trajectory $\tau_i = \{(s_0, a_0), (s_1, a_1), ..., (s_T, a_T)\}$ consists of state-action pairs, the goal is to recover the reward function $r: \mathcal{S} \times \mathcal{A} \to \mathbb{R}$ that the demonstrator is optimizing \citep{NgRussell2000}.

\subsubsection{Max-Margin IRL}

\citet{NgRussell2000} introduced the max-margin approach, establishing that a reward function should make the expert's policy better than alternatives by some margin. For all states $s \in \mathcal{S}$ and actions $a \neq \pi_E(s)$, it enforces constraints:

$$(P_{\pi_E(s)} - P_a)(I - \gamma P_{\pi_E})^{-1}\mathbf{r} \geq 0$$

\noindent where $P_a$ is the transition matrix for action $a$, $P_{\pi_E}$ is the transition matrix under the expert policy, $\gamma$ is the discount factor, and $\mathbf{r} \in \mathbb{R}^{|\mathcal{S}||\mathcal{A}|}$ is the vectorized reward function. The objective maximizes this margin while keeping rewards bounded:

$$\max_{\mathbf{r}} \sum_{s\in \mathcal{S}} \min_{a\neq\pi_E(s)} [(P_{\pi_E(s)} - P_a)(I - \gamma P_{\pi_E})^{-1}\mathbf{r}] - \lambda\|\mathbf{r}\|_1$$
...
\citet{Ho2016} reframed IRL as a distribution matching problem using adversarial training. GAIL employs a discriminator network $D_\omega: \mathcal{S} \times \mathcal{A} \rightarrow [0,1]$ to distinguish between expert and learner behaviors, while a policy network $\pi_\theta: \mathcal{S} \rightarrow \Delta(\mathcal{A})$ tries to fool the discriminator:
