...
the full MDP than FQE. An OLS panel is shown alongside as a definitional
identity reference (r(MSE, R^2) = -1 by construction on a fixed test set).

NOTE on the target operator. Fujimoto (2022) Section 2 defines BRM/FQE with
the policy-evaluation target Q(s', a' ~ pi). This script uses the
Bellman-optimality target max_{a'} Q(s', a'), i.e. FQI-flavored variants of
both algorithms. Because the offline dataset is generated by the optimal
policy, the two targets coincide on the data manifold; they diverge
off-manifold, where value error is evaluated. The qualitative pattern
(BRM zeros BE on D while keeping VE high; FQE retains BE on D with similar
VE) survives the substitution.
"""
...
SEEDS = [42, 123, 777, 314, 271, 1618, 1000, 2718, 9001, 31415]
...
def run_ols(k_grid, seed=42):
    ...
    NOTE: On a fixed test set with fixed SS_tot, R^2 = 1 - n_test * MSE / SS_tot
    is an exact affine transform of MSE, so Pearson r(MSE, R^2) = -1 by
    construction. This panel is plotted as a definitional reference, not an
    empirical finding; it contrasts with the BRM/FQE panel where BE and VE
    do not co-move.
    """
    np.random.seed(seed)
