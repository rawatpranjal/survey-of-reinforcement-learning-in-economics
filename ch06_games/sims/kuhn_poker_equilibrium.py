# Kuhn Poker Equilibrium Computation — Chapter 6, RL in Games
...
from sims.plot_style import apply_style, COLORS, BENCH_STYLE, FIG_SINGLE, FIG_TRIPLE
...
CONFIG = {
    "num_iterations": 2000,
    "num_seeds": 10,
    "version": 2,  # v2: exact best-response exploitability (pure-strategy enumeration)
}
...
# =============================================================================
# EXPLOITABILITY COMPUTATION (exact best response)
# =============================================================================

P0_INFO_SETS = ["J", "Q", "K", "Jpb", "Qpb", "Kpb"]
P1_INFO_SETS = ["Jp", "Qp", "Kp", "Jb", "Qb", "Kb"]
...
def compute_exploitability(strategy: Dict[str, np.ndarray]) -> float:
    ...
    Each BR value is exact: the maximum over all pure strategies of the
    best-responding player (2^6 = 64 per player), so opponent cards are
    implicitly weighted by their reach probabilities. A per-info-set
    greedy BR with uniform card weights understates the true BR value
    and produces spurious near-zero exploitability readings.

    At Nash equilibrium, exploitability = 0.
    """
    expl = 0.0
    for br_player in [0, 1]:
        expl += compute_br_value(strategy, br_player)
    return expl


def compute_br_value(opp_strategy: Dict[str, np.ndarray], br_player: int) -> float:
    """Exact best-response value: enumerate all pure strategies of br_player."""
    info_sets = P0_INFO_SETS if br_player == 0 else P1_INFO_SETS
    best = -np.inf
    for actions in product([0, 1], repeat=len(info_sets)):
        pure = {I: np.array([1.0 - a, float(a)]) for I, a in zip(info_sets, actions)}
        total_ev = 0.0
        for my_card in CARDS:
            for opp_card in CARDS:
                ...
                total_ev += (
                    compute_value_with_strategies(
                        opp_strategy, pure, br_player, cards, ""
                    )
                    / 6
                )
        best = max(best, total_ev)
    return best
...
def compute_value_with_strategies(opp_strategy, my_strategy, br_player, cards, history):
    ...
    for a in [0, 1]:
        ...
        value += probs[a] * compute_value_with_strategies(
            opp_strategy, my_strategy, br_player, cards, new_h
        )
    return value
...
def nash_equilibrium_strategy(alpha: float = 0.0) -> Dict[str, np.ndarray]:
    ...
    Alpha parameterizes player 0's family of equilibria, alpha in [0, 1/3].
    Player 1's equilibrium strategy is unique and does not depend on alpha.
    Exact-BR exploitability of this profile is 0 for every alpha in [0, 1/3].
    """
    return {
...
class CFR:
    ...
    def cfr(
        self, cards: Tuple[str, str], history: str, reach: np.ndarray
    ) -> np.ndarray:
        if is_terminal(history):
            payoff = get_payoff(cards, history)
...
class CFRPlus:
    ...
    def cfr(
        self, cards: Tuple[str, str], history: str, reach: np.ndarray, t: int
    ) -> np.ndarray:
        if is_terminal(history):
            payoff = get_payoff(cards, history)
            return np.array([payoff, -payoff])
...
class FictitiousPlay:
    ...
    def __init__(self):
        self.action_counts: Dict[str, np.ndarray] = defaultdict(
            lambda: np.ones(NUM_ACTIONS)
        )
    ...
    def get_br_action(
        self,
        opp_strategy: Dict[str, np.ndarray],
        br_player: int,
        my_card: str,
        history: str,
    ) -> int:
        ...
        for opp_card in CARDS:
            ...
            for action in [0, 1]:
                ...
                ev = self.evaluate_subtree_optimal(
                    opp_strategy, br_player, cards, new_h
                )
    ...
    def evaluate_subtree_optimal(self, opp_strategy, br_player, cards, history):
        ...
        if player == br_player:
            ...
            for a in [0, 1]:
                new_h = history + ("p" if a == 0 else "b")
                values.append(
                    self.evaluate_subtree_optimal(opp_strategy, br_player, cards, new_h)
                )
            return max(values)
        else:
            opp_card = cards[1 - br_player]
            ...
            for a in [0, 1]:
                new_h = history + ("p" if a == 0 else "b")
                value += probs[a] * self.evaluate_subtree_optimal(
                    opp_strategy, br_player, cards, new_h
                )
            return value
...
def generate_exploitability_plot(aggregated, save_path):
    ...
    colors = {"CFR": COLORS["blue"], "CFR+": COLORS["green"], "FP": COLORS["orange"]}
    labels = {"CFR": "Vanilla CFR", "CFR+": "CFR+", "FP": "Fictitious Play"}

    # Rank order by final exploitability (best first)
    for method in ["CFR+", "CFR", "FP"]:
        ...
        ax.semilogy(
            iters, mean, color=colors[method], label=labels[method], linewidth=1.5
        )
        ax.fill_between(
            iters,
            np.maximum(np.array(mean) - np.array(std), 1e-6),
            np.array(mean) + np.array(std),
            color=colors[method],
            alpha=0.2,
        )

    ax.axhline(y=0.01, **BENCH_STYLE, label="ε = 0.01")
...
def generate_strategy_plot(aggregated, save_path):
    ...
    for ax, method, title in zip(axes, methods, titles):
        ...
        learned_probs = [
            strategy.get(is_, np.array([0.5, 0.5]))[1] for is_ in info_sets
        ]
...
def generate_results_table(aggregated, save_path):
    ...
    for method, label in [
        ...
        lines.append(
            f"{label} & {' & '.join(expl_vals)} & {conv_str} & {time_str} \\\\"
        )
...
def compute_data():
    ...
    results = run_experiment(
        num_iterations=CONFIG["num_iterations"], num_seeds=CONFIG["num_seeds"]
    )
    ...
    for method in ["CFR", "CFR+", "FP"]:
        ...
        conv = find_convergence_iteration(
            d["iterations"], d["mean_exploitability"], 0.01
        )
        print(f"\n{method}:")
    ...
    for method, d in aggregated.items():
        ...
        if sd["final_strategy"] is not None:
            sd["final_strategy"] = {
                k: v.tolist() for k, v in sd["final_strategy"].items()
            }
...
def _deserialize_aggregated(aggregated):
    ...
    for method, d in aggregated.items():
        ...
        if dd["final_strategy"] is not None:
            dd["final_strategy"] = {
                k: np.array(v) for k, v in dd["final_strategy"].items()
            }
...
def generate_outputs(data):
    ...
    generate_exploitability_plot(
        aggregated, f"{output_dir}/kuhn_poker_exploitability.png"
    )
