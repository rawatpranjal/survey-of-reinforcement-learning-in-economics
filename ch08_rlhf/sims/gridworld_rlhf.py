...
Compares five methods on a 10x10 stochastic gridworld with hazards and traps:
...
def policy_evaluation(env, policy, reward_fn=None, tol=1e-10, max_iter=5000):
    """Evaluate V^pi(s) for a deterministic policy under given reward function."""
    nS = env.num_states
    V = np.zeros(nS)
    if reward_fn is None:
        reward_fn = env.true_reward
    for it in range(max_iter):
        ...
        for si in range(nS):
            ...
            s = env.index_to_state(si)
            a = int(policy[si])
            V_new[si] = sum(p * (reward_fn(s, a, ns) + GAMMA * V[env.state_to_index(ns)])
                            for ns, p in env._trans_cache[(si, a)])
        if np.max(np.abs(V_new - V)) < tol:
            break
        V = V_new
...
# =============================================================================
# Output Generation: Environment Figure and Diagnostics
# =============================================================================

def generate_env_figure(env, V_star, pi_star):
    """Generate environment diagram with V* heatmap and optimal policy arrows."""
    ...
    # Arrow directions: (dx_col, dy_row) in plot coords
    # Actions: 0=up(-1,0), 1=down(+1,0), 2=left(0,-1), 3=right(0,+1)
    # imshow origin='upper': y increases downward, x increases rightward
    arrow_map = {0: (0, -0.3), 1: (0, 0.3), 2: (-0.3, 0), 3: (0.3, 0)}

    for r in range(env.N):
        for c in range(env.N):
            si = env.state_to_index((r, c))
            if (r, c) == env.goal:
                continue
            a = pi_star[si]
            dx, dy = arrow_map[a]
    ...
    ax.text(0, 0, 'S', ha='center', va='center',
            fontsize=11, color='white', fontweight='bold')
    for h in HAZARDS:
        ax.text(h[1], h[0], 'H', ha='center', va='center',
                fontsize=11, color='#ff6666', fontweight='bold')
    for t in TRAPS:
        ax.text(t[1], t[0], 'T', ha='center', va='center',
                fontsize=11, color='#ff6666', fontweight='bold')

    ax.set_xlabel('Column')
    ax.set_ylabel('Row')

    fig_path = os.path.join(OUTPUT_DIR, 'gridworld_rlhf_env.png')
...
def generate_diagnostics(env, V_star, pi_star):
    """Compute diagnostics at K=5000 for one seed and write LaTeX table."""
    K = 5000
    ...
    comps = generate_comparisons(env, K)
    mask = np.arange(env.num_states) != env.goal_si

    # True expected reward table
    R_true = np.zeros((env.num_states, env.num_actions))
    for si in range(env.num_states):
        s = env.index_to_state(si)
        if si == env.goal_si:
            continue
        for a in range(env.num_actions):
            for ns, p in env._trans_cache[(si, a)]:
                R_true[si, a] += p * env.true_reward(s, a, ns)

    def _reward_table(rfn):
        R = np.zeros((env.num_states, env.num_actions))
        for si in range(env.num_states):
            s = env.index_to_state(si)
            if si == env.goal_si:
                continue
    ...
    # NN RLHF
    model, _ = train_reward_net(list(comps))
    R_nn = extract_reward_table(model, env)
    _, pi_nn = vi_from_reward_table(env, R_nn)
    V_nn = policy_evaluation(env, pi_nn)
    results.append(('NN RLHF',
                     np.mean(pi_nn[mask] == pi_star[mask]) * 100,
                     np.mean(np.abs(R_nn[mask] - R_true[mask])),
                     np.corrcoef(V_nn[mask], V_star[mask])[0, 1],
                     count_hazard_visits(env, pi_nn)))

    # Correct structural
    phi_c = fit_structural(comps, env, 4)
    _, pi_c, _ = value_iteration(env, structural_reward_fn(env, phi_c, 4))
    V_c = policy_evaluation(env, pi_c)
    R_co = _reward_table(structural_reward_fn(env, phi_c, 4))
    results.append(('Correct',
                     np.mean(pi_c[mask] == pi_star[mask]) * 100,
                     np.mean(np.abs(R_co[mask] - R_true[mask])),
                     np.corrcoef(V_c[mask], V_star[mask])[0, 1],
                     count_hazard_visits(env, pi_c)))

    # Misspecified structural
    phi_m = fit_structural(comps, env, 2)
    _, pi_m, _ = value_iteration(env, structural_reward_fn(env, phi_m, 2))
    V_m = policy_evaluation(env, pi_m)
    R_mi = _reward_table(structural_reward_fn(env, phi_m, 2))
    results.append(('Misspecified',
                     np.mean(pi_m[mask] == pi_star[mask]) * 100,
                     np.mean(np.abs(R_mi[mask] - R_true[mask])),
                     np.corrcoef(V_m[mask], V_star[mask])[0, 1],
                     count_hazard_visits(env, pi_m)))

    # Print
    print('\n' + '=' * 80)
    print('DIAGNOSTICS (K=5000, single seed)')
    print('=' * 80)
    hdr = f'  {"Method":<15s} | {"Agree%":>7s} | {"R MAE":>7s} | {"V corr":>7s} | {"Haz.vis":>7s}'
    print(hdr)
    print('-' * len(hdr))
    for name, agree, mae, vcorr, hv in results:
        mae_str = f'{mae:.4f}' if mae is not None else '    ---'
        print(f'  {name:<15s} | {agree:7.1f} | {mae_str:>7s} | {vcorr:7.4f} | {hv:7d}')

    # Write LaTeX table
    tex_path = os.path.join(OUTPUT_DIR, 'gridworld_rlhf_diagnostics.tex')
    with open(tex_path, 'w') as f:
        f.write('\\begin{tabular}{lrrrr}\n\\hline\n')
        f.write('Method & Policy agreement (\\%) & Reward MAE '
                '& $V^\\pi$ corr.\\ with $V^*$ & Hazard visits \\\\\n\\hline\n')
        for name, agree, mae, vcorr, hv in results:
            mae_str = f'${mae:.3f}$' if mae is not None else '---'
            f.write(f'{name} & ${agree:.1f}$ & {mae_str} & ${vcorr:.3f}$ & ${hv}$ \\\\\n')
        f.write('\\hline\n\\end{tabular}\n')
...
def run_experiment():
    ...
    print(f'  6. Online >= offline: {on_mean:.3f} vs {off_mean:.3f}, p={p_val:.4f} -> '
    ...
    tex3 = os.path.join(OUTPUT_DIR, 'gridworld_rlhf_diagnostics.tex')
    env_fig = os.path.join(OUTPUT_DIR, 'gridworld_rlhf_env.png')
    print(f'\nOUTPUT FILES:')
    print(f'  {env_fig}')
    print(f'  {fig_path}')
    print(f'  {tex1}')
