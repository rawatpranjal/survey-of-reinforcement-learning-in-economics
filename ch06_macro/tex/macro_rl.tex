...
% PROSE PLACEHOLDER (Pass 2): Anchor AS_IMF (Atashbar2023imf) on
% representative-agent RBC with PPO + DDPG vs projection +
% perturbation. Tier-B supporting Shi2022consumption (single-agent
% consumption-savings actor-critic, sets the explore-vs-exploit
% baseline). One framing paragraph + AS_IMF result + one-sentence
% SHI cite. ~1pp.

(prose to be drafted in Pass 2)
...
% PROSE PLACEHOLDER (Pass 2): Anchor Mi2025dsmfg. Frame as the
% leader-follower variant of MFG (RSPG above is the symmetric case).
% Government-as-leader sets policy first, continuum of followers
% responds; SMFRL algorithm uses CTDE alternation; scales to 1000
% agents; tax-policy benchmark beats Saez analytical and 2022 US
% federal income tax. Bridges into single-planner section §5.2.
% ~1pp.
...
% PROSE PLACEHOLDER (Pass 2): Anchor Kaplowitz2025 (KAP) — Q-learning
% household reproduces MPC heterogeneity (Ganong et al. 2024) and
% scarring (Malmendier-Shen 2024), two empirical anomalies that
% rational expectations + borrowing constraints cannot generate
% jointly. Initialised at RE solution; deviations from learning
% dynamics. Tier-B supporting: Shi2022consumption (single-agent RL
% converges to RE asymptotically — sets the baseline KAP departs
% from), Kuriksha2021 (heterogeneous-experience aggregation, fat-tailed
% wealth distributions). ~2pp.
...
Sections~\ref{sec:macro:solver_single_agent}--\ref{sec:macro:policy} report
