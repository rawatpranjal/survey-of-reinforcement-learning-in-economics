...
The implementation follows MFAX in evaluating the reward after the
population transition, using $\bar{s}_{t+1}$, and using the terminal
reward on the final transition. The reward encourages agents to
coordinate with the moving population while paying an action cost.
...
history helps infer where the common-noise episode is headed. Expected
returns are reported for completeness but should not be read as a
welfare ranking across methods, since each method induces a different
mean-field path. Figure~\ref{fig:macro:lqmfg-curves} plots
