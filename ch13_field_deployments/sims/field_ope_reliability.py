...
def write_candidates_table(data, path):
    ...
    lines = [
        ...
        r"under-valued); importance-sampling effective sample size (ESS) is $(\sum w)^2/\sum w^2$ over the log's "
        + str(RUN_CFG["n_traj"])
        ...
        r"\toprule",
        r" & & \multicolumn{3}{c}{DM error} & \multicolumn{3}{c}{IS ESS} \\",
        r"\cmidrule(lr){3-5}\cmidrule(lr){6-8}",
        r"Candidate & True & A/B & Inc. & Mix. & A/B & Inc. & Mix. \\",
        r"\midrule",
    ]
    for nm in order:
        s = stats[nm]
