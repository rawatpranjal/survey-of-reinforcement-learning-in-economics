...
\noindent The trajectory is continuing rather than episodic. The paper marks no episode boundary and trains continuously on new events, so the horizon is effectively unbounded and only the discount limits it. Following the policy-gradient conventions of Section~\ref{section:rl_algorithms}, the objective and its REINFORCE gradient are
\begin{equation}
