...
The paper casts this candidate-generation problem as an MDP. The state $s \in \mathcal{S}$ is a continuous representation of the user's recent interaction history and context, implemented with a recurrent recommender model. The action $a \in \mathcal{A}$ is a video selected from the available catalog. The reward $r(s,a)$ is an immediate user-activity signal, with the live-experiment discussion focusing on ViewTime and related engagement outcomes; the return aggregates future reward with discount factor $\gamma$. The trajectory is continuing rather than episodic, the paper marks no episode boundary and trains continuously on new events, so the horizon is effectively unbounded and only the discount limits it. In compact form, the policy-gradient objective is
\begin{equation}
J(\theta)
