...
Taobao cold-start ranking & Reinforcement-learning lifetime value (RL-LTV) was deployed on Taobao for cold-start recommendation, with live A/B results \citep{Ji2021RLLTV}. \\
...
Alibaba sponsored-search real-time bidding (RTB) & Robust Markov decision process (robust MDP) bidding control was deployed and tested in Alibaba's search auction platform \citep{Zhao2018SponsoredSearchRTB}. \\
...
DeepStock inventory & Deep reinforcement learning (DRL) with inventory-policy regularization was rolled out across Tmall stock-keeping-unit (SKU) warehouse replenishment \citep{Xie2026DeepStock}. \\
...
InstructGPT RLHF & Proximal policy optimization (PPO) against a learned reward model shaped deployed language-model behavior, training-time RL rather than inference-time control \citep{ouyang2022training}. \\
BCOOLER heating, ventilation, and air-conditioning (HVAC) & RL produced live supervisory-control recommendations at two commercial buildings, field trials without broad rollout \citep{Luo2022BCOOLER}. \\
Tmall pricing, hotel revenue management & Real field experiments controlled prices \citep{Liu2019} or revenue-management decisions \citep{Chen2023hotelrl}, without broad rollout evidence. \\
Financial execution, RTB simulator, inventory benchmark & Real data \citep{Nevmyvaka2006execution} or benchmark evidence \citep{Wu2018rtb,Gijsbrechts2022inventory} without public proof of live deployed control. \\
\bottomrule
\end{tabular}
...
The deployment evidence comes from the official joint DeepMind and YouTube post \citep{DeepMind2022MuZeroRCPost}, which reports production launch on a portion of YouTube live traffic and an average bitrate reduction. This chapter treats MuZero-RC as the canonical physical or design runtime-control case: it replaced a narrow codec control component, not the whole video platform.
