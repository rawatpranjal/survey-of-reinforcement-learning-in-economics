...
the sup-norm.\footnote{\citet{Iyengar2005} establishes this via a direct epsilon-argument, the same swap-style technique as the proof of Lemma~\ref{lem:bellman_contraction} (Section~\ref{sec:planning_learning}). An equivalent route is Blackwell's sufficient conditions (Theorem~\ref{thm:blackwell}): monotonicity holds because every $p \in \mathcal{P}(s,a)$ is nonnegative, and discounting holds with equality, $T(V+c) = TV + \gamma c$, because every $p \in \mathcal{P}(s,a)$ sums to one, so the constant shift passes unchanged through both the inner minimum and the outer maximum.} So robust value iteration and policy iteration converge
