...
The IQN-Neutral policy recovers 86\% of the DP oracle's mean return and
IQN-CVaR$_{95}$ recovers 90\%. Across ten seeds the CVaR$_{95}$
evaluation improves the average left tail relative to the risk-neutral
evaluation (CVaR$_{95}$ of $-55.3$ against $-57.5$) with no loss in
mean return (36.4 against 34.9, a gap of under two standard errors),
and its ordering policy tracks the oracle more closely at high
inventory levels (Figure~\ref{fig:risk_inventory_policy}). Both learned
policies order more than the oracle, averaging order quantities across
inventory levels (2.99 and 3.21 against the oracle's 2.56).
...
on~\eqref{eq:dual_update}. Each Q-learning variant is run
over ten seeds. The table reports means and standard
errors.\footnote{The LP oracle value is computed by exact
policy evaluation
$V = (I - \gamma P_\pi)^{-1} R_\pi$, an infinite-horizon
quantity. The Q-learning returns are Monte Carlo averages
over rollouts truncated at horizon $H = 100$. With
$\gamma^{100} \approx 6\times 10^{-3}$, the truncation
removes a non-negative discounted tail bounded by roughly
$\gamma^{H} r_{\max}/(1-\gamma)$, on the order of one to two
percent of the return. Part of the gap between the LP value
and the Q-learning returns is therefore truncation bias
rather than policy suboptimality.}
Unconstrained Q-learning converges
near the DP optimum (return $258 \pm 3$ versus 273) but
violates the carbon budget by a factor of three
(cost $99 \pm 3$ versus $d = 31$).
The learned multiplier rises from zero and settles at
$\lambda \approx 1.40$, near the analytical shadow price
$\lambda^* = 1.20$. Across the ten seeds the trajectory
is nearly monotone: the peak value is $1.405 \pm 0.002$,
within one percent of the settling value, so naive dual
ascent here exhibits little of the overshoot that
\citet{Stooke2020}'s PID controller is designed to dampen.
The final $\lambda$ sits about sixteen percent above
$\lambda^*$, and the corresponding policy holds emissions
at $28 \pm 4$, below the budget of 31, leaving the
Lagrangian agent slightly too conservative
(return $178 \pm 4$ versus the LP optimum of 186).
The LP optimal policy is stochastic,
...
the sup-norm.\footnote{\citet{Iyengar2005} establishes this via a direct epsilon-argument, the same swap-style technique as the proof of Lemma~\ref{lem:bellman_contraction} (Section~\ref{sec:planning_learning}). The contraction result also follows from Blackwell's sufficient conditions (Theorem~\ref{thm:blackwell}). Every $p \in \mathcal{P}(s,a)$ is nonnegative, which gives monotonicity. Every $p \in \mathcal{P}(s,a)$ also sums to one, so the constant shift passes unchanged through both the inner minimum and the outer maximum. Discounting therefore holds with equality, $T(V+c) = TV + \gamma c$.} So robust value iteration and policy iteration converge
...
models, with each of the seven policies evaluated on a common set of ten
income-sequence seeds so that returns are mean estimates with standard
errors. Under the nominal model, all policies perform similarly.
Under the perturbed model, standard DP degrades by 76\% while the
$\theta = 2$ robust policy degrades by 72\%. Across the ten seeds the
Q-learning and robust Q-learning policies recover their DP counterparts:
the two robust variants reproduce the robust DP consumption rule exactly,
and standard Q-learning differs at no more than one wealth level, with
returns statistically indistinguishable from standard DP. The model-free
algorithms thus recover the same precautionary savings behavior without
access to the income distribution.
The gain from robustness here is modest in absolute terms. The
$\theta = 2$ policy recovers about 4.5 percentage points of the 76\%
degradation, whereas an oracle that knows the perturbed distribution
recovers 37 points. KL-ball robustness hedges against a band of nearby
distributions rather than fitting the realized one, so it closes only a
fraction of the oracle gap, the expected price of not knowing the
true model.
Figure~\ref{fig:robust_consumption} shows the mechanism, where robust agents
