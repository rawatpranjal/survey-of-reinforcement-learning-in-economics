    ...
    def _precompute_demand(self):
        ...
        for prob, lam in zip(self.demand_probs, self.demand_lambdas):
            for d in range(max_d + 1):
                ...
                self.demand_pmf[d] += prob * (lam**d * np.exp(-lam) / factorial(d))
...
class IQNNetwork(nn.Module):
    def __init__(self, state_dim, n_actions, layer_size=128, n_cos=64):
        ...
        self.pis = torch.FloatTensor([np.pi * i for i in range(1, n_cos + 1)]).view(
            1, 1, n_cos
        )
...
def train_iqn(env, config, seed):
    ...
    for episode in range(config["N_TRAIN_EPISODES"]):
        ...
        for t in range(env.horizon):
            ...
            buffer.add(state / env.s_max, action, reward, next_state / env.s_max, done)
            ...
            if (
                ...
                with torch.no_grad():
                    ...
                    q_next_best = q_next[
                        torch.arange(config["BATCH_SIZE"]), :, best_actions
                    ]
                    targets = (
                        r_t.unsqueeze(1)
                        + config["GAMMA"] * (1 - d_t.unsqueeze(1)) * q_next_best
                    )

                q_current, taus = net(s_t, n_tau)
                q_current_a = q_current[torch.arange(config["BATCH_SIZE"]), :, a_t]
                ...
                quantile_loss = (
                    tau_vals - (td_error.detach() < 0).float()
                ).abs() * huber
...
def iqn_act(
    net, state_scalar, s_max, tau_mode="uniform", alpha=1.0, n_tau_eval=64, rng=None
):
...
def generate_outputs(data):
    ...
    # ── Table ────────────────────────────────────────────────────────────
    show_se = n_seeds > 1  # SE across a single seed is identically zero
    ...
    header = (
        f"{'Policy':<16} {'Mean':>8}"
        + (f" {'SE':>6}" if show_se else "")
        + f" {'CVaR95':>8} {'CVaR99':>8} {'Avg Ord':>8}"
    )
    print(header)
    print("-" * 72)

    tex_lines = [
        r"\begin{tabular}{l" + "c" * (5 if show_se else 4) + "}",
        r"\hline",
        r"Policy & Mean Return & "
        + (r"SE & " if show_se else "")
        + r"CVaR$_{95}$ & CVaR$_{99}$ & Avg.\ Order \\",
        r"\hline",
    ]
    ...
    for name in policy_names:
        ...
        print(
            f"{name:<16} {mean_val:>8.1f}"
            + (f" {se_val:>6.1f}" if show_se else "")
            + f" {cvar95_val:>8.1f} {cvar99_val:>8.1f} {avg_ord_val:>8.2f}"
        )
        tex_lines.append(
            f"{name} & {mean_val:.1f} & "
            + (f"{se_val:.1f} & " if show_se else "")
