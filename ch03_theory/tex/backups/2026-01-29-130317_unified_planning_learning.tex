...
Fifth, the Newton framework yields quantitative bounds on approximation error. \citet{bertsekas2022} (Proposition 2.2.1) proves that if $\tilde{\pi}$ is the one-step lookahead policy based on approximation $\tilde{J}$, then
\[
\|J^{\tilde{\pi}} - J^*\|_v \leq \frac{2\gamma}{1-\gamma} \|\tilde{J} - J^*\|_v.
\]
The amplification factor $2\gamma/(1-\gamma)$ has concrete numerical consequences. At $\gamma = 0.95$, a 1\% approximation error produces at most 38\% policy cost error. At $\gamma = 0.99$, the same 1\% error amplifies to 198\%. This bound quantifies the ``region of convergence'' concept: the approximation $\tilde{J}$ must be accurate enough that the amplified error remains acceptable.

For finite tabular MDPs, the sample complexity gap between model-based and model-free methods has been precisely characterized.\footnote{These bounds apply specifically to finite tabular MDPs and may not extend to function approximation settings.} \citet{li2024minimax} show that Q-learning requires $\tilde{O}(|\mathcal{S}||\mathcal{A}|/(1-\gamma)^4\varepsilon^2)$ samples to achieve $\varepsilon$-optimal policy, while model-based methods require $\tilde{O}(|\mathcal{S}||\mathcal{A}|/(1-\gamma)^3\varepsilon^2)$ samples. The $(1-\gamma)^{-1}$ gap reflects the difference between Newton-type and gradient-type convergence in the statistical setting.
...
\input{../ch02_planning_learning/sims/ssp_gridworld_results.tex}
\end{table}
