...
# Closed-form calculator on the Engine Replacement MDP. Under the one-logit-per-state
...
def generate_outputs(data):
    ...
    with open(tex_path, "w") as f:
        ...
        f.write(
            "\\caption{Policy-gradient objects on the Engine Replacement MDP at the uniform "
        ...
        pg_count = (
            str(it["PG"]) if it["PG"] is not None else f"not reached in {N_STEPS}"
        )
        npg_count = (
            str(it["NPG"]) if it["NPG"] is not None else f"not reached in {N_STEPS}"
        )
        f.write(
            "policy-gradient iterations to $J^\\star-J<10^{-3}$ "
            f"& \\multicolumn{{2}}{{c}}{{{pg_count}}} \\\\\n"
        )
        f.write(
            "natural-gradient iterations to $J^\\star-J<10^{-3}$ "
            f"& \\multicolumn{{2}}{{c}}{{{npg_count}}} \\\\\n"
        )
        f.write("\\hline\n\\end{tabular}\n\\end{table}\n")
...
def main():
    parser = argparse.ArgumentParser(
        description="Policy square on the Engine Replacement MDP"
    )
