...
The defining restriction is the fixed dataset, not the provenance of the data. A simulator can generate the dataset, which is then frozen and used to search for an improved policy, and that exercise is offline; field logs can be used to fit a policy that is then allowed to explore, and that exercise ceases to be offline once the exploration begins. The distinction is familiar in econometrics, where the analyst works with one sample that was collected under a regime nobody chose and cannot commission a new one, while an online learner is closer to a researcher who designs the next experiment after reading the results of the last. Online learning dominates when exploration is cheap, because only a policy that can be run can be evaluated without extrapolation, and the advantage disappears when running a new policy is expensive or unsafe.

The domains covered elsewhere in this survey divide along these lines. Game engines are predominantly online, since a simulator makes exploration nearly free and self-play generates its own data (Section~\ref{section:rl_games}); the historically important exceptions bootstrap from a corpus of recorded human play before switching to self-play, which is an offline-to-online sequence. Robotics is hybrid, with training in simulation followed by transfer to hardware, because sustained online exploration on a physical machine is slow and destructive (Section~\ref{section:field_deployments}). Language-model post-training runs both ways, with direct preference optimization fitting a fixed set of preference comparisons and the policy-gradient variants of reinforcement learning from human feedback generating fresh samples from the current policy (Section~\ref{section:rlhf}). Recommendation and dispatch systems at scale generally begin offline on logs of previous serving policies and then admit limited live exploration (Section~\ref{section:field_deployments}). Pricing sits mostly with contextual bandits run online, with the dynamic and therefore genuinely sequential problems the exception rather than the rule (Section~\ref{section:bandits}). The visibility of the online cases owes something to the salience of the game results, and not to any general superiority of online learning in settings where exploration carries a cost.
...
Pessimism-based offline RL centers on the value function, with the policy emerging from a Q-function constructed to be conservative outside the data. An alternative inverts that relationship. Train a supervised or generative model on the logged trajectories themselves, and treat the model as the policy. The family includes two designs at opposite ends: a transformer-based trajectory model and a stripped-down MLP that isolates the operative ingredient.
...
This chapter therefore concerns fixed-data policy improvement when scalar rewards are observed. This is distinct from RLHF, where the reward itself must be inferred from preference comparisons and then interpreted as an object for aggregation, incentives, and welfare. The next chapter takes up that separate problem.
