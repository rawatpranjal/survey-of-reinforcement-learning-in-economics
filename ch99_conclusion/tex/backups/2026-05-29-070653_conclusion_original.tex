...
Reinforcement learning enters this picture not as a third culture but as a computational toolkit that relaxes the information requirements of dynamic programming while preserving its mathematical foundations.
...
TD-based methods make structural models tractable at scales where the nested fixed-point algorithm is infeasible, as the simulation in Section~\ref{section:rl_econ_models} demonstrates: a DQN trained on the bus-engine replacement environment matches the analytical optimum within two percent at state-space sizes where value iteration exhausts memory \citep{Rust1987}.
...
The dispatch, revenue management, and inventory simulations of Section~\ref{section:applications} show that RL can compute practically deployable policies in domains where neither analytical solutions nor small-state DP are available.

Multi-agent settings expose a second contribution.
...
Offline reinforcement learning (Section~\ref{section:offline_rl}) addresses the setting most familiar to economists: the analyst has a fixed dataset of historical decisions and must improve on the behavioral policy without further experimentation.
...
Robust and constrained RL (Section~\ref{section:dist_robust_constrained}) is a third locus of cross-pollination.
...
Industrial deployments described in Section~\ref{section:applications} required dedicated engineering teams and months of hyperparameter search; the academic simulations here required comparable effort at much smaller scale.
...
The structural equivalences documented in Section~\ref{subsec:structural_equivalences}, the softmax-logit identity, the Emax-soft-value correspondence, the Q-function-CCP duality, are not coincidences; they reflect the fact that optimal sequential decision-making under uncertainty is one problem, and the two traditions have been working toward it from opposite ends.
