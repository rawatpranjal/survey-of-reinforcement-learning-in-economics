...
|---|---|---|---|
| `murphy2003dtr` | A DTR is a sequence of history-dependent rules; observed-data backward induction identifies an optimal benefit-to-go under no unmeasured confounding. Fast Track randomizes a complex prevention program against control, while its home-visiting component uses an end-of-semester six-item family-functioning score to set the following semester's visit level. Lower scores indicate greater need, and the team avoids assigning the highest level to every family because of concerns about dependency, pejorative labeling, and attrition. The observable sequence is pretreatment status, successive treatment decisions and statuses, and a terminal response; only the potential variables under the realized treatment sequence are observed. The later status is an intermediate outcome of past treatment, so the setup permits delayed treatment effects. Murphy also explains that treatment groups can differ in composition when relevant confounders are omitted. | `murphy2003dtr.md`, opening Fast Track discussion and Section 2. The trial arms, intervention aims, semester timing, questionnaire, assignment rule, and overtreatment concerns appear together in the opening discussion. Section 2 defines the observable sequence, indexes later states by earlier treatment, connects realized values to potential states and outcomes, and gives the composition argument before stating no unmeasured confounding. | Retained. The chapter distinguishes the randomized effect of the full program from the adaptive home-visiting question. It keeps the terminal response generic because Murphy's formal notation does not select one Fast Track endpoint. Its theorem adds explicit standard-Borel, positivity, terminal-kernel, and tie-breaking conditions rather than attributing those details to Murphy. The unadjusted visit-count regression is a chapter benchmark, not a method attributed to Murphy. |
| `schulte2014qlearning` | Q-learning is backward recursive outcome regression for DTRs; A-learning estimates treatment contrasts; the notation connects DTR and RL methods. The STAR*D application analyzes levels 2 and 3 as two stages using 795 complete records, with 330 patients continuing to stage 2. Switch versus augmentation is observational even though specific treatments are randomized within patient preference. Q-learning recommends switching above a first-stage QIDS-slope threshold of `-1.09` and switching at stage 2. Its stage-2 switch contrast is `1.10` with 95% interval `[0.02,2.19]`; the first-stage slope interaction is `1.02` with interval `[-0.08,2.11]`. | `schulte2014qlearning.md`, Sections 2-5 for the assumptions and recursion and Section 7 for STAR*D, especially the design description, complete-case sample, outcome definition, Table 2, and fitted rules. | Retained as the foundational bridge and added as the expository example after the theorem. The chapter states that the binary switch-versus-augmentation action requires stable versions, that exchangeability is not guaranteed by the within-preference randomization, and that complete-case analysis adds a dropout condition. It does not claim a policy-value interval or prospective validation. |
...
| Citation | Manuscript claim | Full-text location checked | Disposition |
|---|---|---|---|
| `laber2014dtrchallenges` | The ADHD SMART analysis retains 138 of 155 children, 81 of whom receive a second randomization; it uses the 32-week teacher Impairment Rating Scale outcome. The fitted rules depend on adherence and prior medication exposure. Adaptive 90% intervals do not support a unique first-stage recommendation but do distinguish the stage-2 actions at low adherence, with intervals `[-2.21,-0.57]` and `[-2.51,-0.60]`; near-zero contrasts create nonregular inference. | `laber-2014-dtr-technical-challenges.md`, application at pp. 23-27, especially Tables 6-9, and open problems at pp. 27-29. | Retained as the application for inference after policy learning. The prose distinguishes an estimated argmax from the evidence for its prescribed action and separates contrast inference from evaluation of a selected rule. |
