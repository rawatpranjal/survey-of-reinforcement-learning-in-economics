...
|---|---|---|---|---|
| `murphy2003dtr` | Foundational | Defines DTR backward induction and motivates it with Fast Track. The chapter uses the trial's intervention-versus-control design, semester-level family-functioning assessment, adaptive home-visiting rule, and overtreatment concerns to explain why the policy target differs from a fixed-protocol contrast. It also maps Murphy's observed sequence and potential variables to the running example without selecting a terminal endpoint that the paper leaves generic. The chapter clarifies that an earlier visit may change later family status and that a raw visit-count association is confounded by need. The chapter states its own formal regularity conditions. | `murphy2003dtr.md` | Full text checked |
| `schulte2014qlearning` | Foundational bridge and expository application | Connects causal DTR notation to Q-learning and A-learning. The STAR*D application maps histories, actions, potential states, assumptions, backward regressions, fitted rules, and coefficient intervals to the theorem. The chapter preserves the paper's warning that switch versus augmentation is observational and separates identification from working-model and dropout requirements. | `schulte2014qlearning.md` | Full text checked |
...
| `laber2014dtrchallenges` | Inference application | Three-paragraph ADHD SMART application covers the sample and treatment sequence, fitted rules and adaptive intervals, and the nonregular inference problem after a regime is learned. | `laber-2014-dtr-technical-challenges.md` | Newly sourced; full text checked |
