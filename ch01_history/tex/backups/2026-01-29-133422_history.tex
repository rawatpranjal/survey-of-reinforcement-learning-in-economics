...
\end{equation}
Features included material, mobility, pawn structure, and king safety. Shannon did not implement a chess program; the paper is theoretical, outlining the architecture and key design choices.
...
\end{equation}
where $L$ is a surrogate objective based on the advantage function $A^\pi(s,a) = Q^\pi(s,a) - V^\pi(s)$. The KL constraint prevents the policy from moving so far in one update that performance collapses and recovery becomes impossible.
