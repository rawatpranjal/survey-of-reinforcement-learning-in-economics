...
|---|------|---------|--------|---|
| A1 | ch99 conclusion expand | `ch99_conclusion/tex/conclusion.tex` | DONE 2026-05-22. 33→112 lines, 6 subsections (Two Cultures Revisited / What RL Adds / What Econ Adds / Sim Evidence / Open Challenges / Conclusion). 25 cites verified. | DONE |
| A2 | ch10b chapter intro | `ch10b_rl_for_ci/tex/rl_for_ci.tex` (lines 19-27) | DONE 2026-05-22. 5 paragraphs. 9 cites verified. ch10b PDF 33pp clean. | DONE |
| A3 | ch00 intro + abstract refresh | `ch00_introduction/tex/{intro,abstract,language}.tex` | DONE 2026-05-22. Abstract 150→227 words. Chapter list verified vs main.tex (all 16 chapters). 5 new cites. ch00 PDF 4pp clean. | DONE |
| A4 | ch09 RLHF: Constitutional AI, RLAIF, scalable oversight | `ch09_rlhf/tex/rlhf.tex` §5.3.5 | DONE prior session. Cites bai2022constitutional / lee2023rlaif / bowman2022oversight. | DONE |
| A5 | ch09 RLHF: GARP research-gap paragraph | `ch09_rlhf/tex/rlhf.tex` §5.3.2 line 81 | DONE prior session. Cites golz2025distortion / raheja2026unification / zhang2025omd / liu2024comal / tang2024gpo. | DONE |
...
**B1. fishery_paradigms — CLOSED 2026-05-22.** Sources: `audits/ch12_world_models__fishery_paradigms_reaudit_2026-05-22.md` (30% pre-B1.6) and `audits/ch12_world_models__fishery_paradigms_polish_2026-05-20.md`. B1.6 landed in the same session: `rollout` instrumented with `track_traj` flag, `compute_paradigm` captures seed-0 `s_traj`/`h_traj`, `generate_outputs` produces 1×2 figure (regret panel + stock/harvest panel with K=10 and MSY=1.0 reference lines). Caches deleted + sim rerun; all 8 paradigm caches post-date code. ch12 PDF 3.16MB. Re-audit recommended next session to confirm closure score.
...
| B1.6 | Stock + harvest trajectory figure | LANDED 2026-05-22 |
...
| C4 | refs.bib orphan/missing-key sweep | `docs/refs.bib` | DONE 2026-05-22. 491 entries, 435 cited, 0 missing, 54 expected orphans (world-models queue + macro/games queue). Report `docs/plans/2026-05-22-refs-bib-sweep.md`. Thoeni key-year mismatch FIXED: renamed `Thoeni2026nmfg` → `Thoeni2025nmfg` in both bib + `ch06_macro/tex/macro_rl.tex:177`. | DONE |
