...
where glyph[epsilon1] t is an input noise vector, sampled from some fixed distribution, such as a spherical Gaussian. We can now rewrite the objective in Equation 10 as
...
where a t is evaluated at f φ ( glyph[epsilon1] t ; s t ) . This unbiased gradient estimator extends the DDPG style policy gradients (Lillicrap et al., 2015) to any tractable stochastic policy.
...
Proof. Define the entropy augmented reward as r π ( s t , a t ) glyph[defines] r ( s t , a t ) + E s t +1 ∼ p [ H ( π ( · | s t +1 ))] and rewrite the update rule as
...
It must be the case that J π old ( π new ( · | s t )) ≤ J π old ( π old ( · | s t )) , since we can always choose π new = π old ∈ Π . Hence E a t ∼ π new [log π new ( a t | s t ) -Q π old ( s t , a t ) + log Z π old ( s t )] ≤ E a t ∼ π old [log π old ( a t | s t ) -Q π old ( s t , a t ) + log Z π old ( s t )]
