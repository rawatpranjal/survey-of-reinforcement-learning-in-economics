...
# Algorithm definitions: (name, runner_function, kwargs)
# Split into learning algorithms and planning algorithms
...
# Planning algorithms (Bertsekas framework)
# These use V_base initialized from partial VI or heuristic

planning_algorithms = {
    'Rollout (heuristic)': {
        'runner': run_rollout,
        'kwargs': {'V_base': None}  # Uses Manhattan heuristic
    },
    'Rollout (VI-5)': {
        'runner': run_rollout,
        'kwargs': {}  # Will set V_base from partial VI
    },
    'Lookahead (l=2)': {
...
# Compute partial VI solutions for planning methods
# Start from Manhattan distance heuristic, then do 5 VI steps to refine
...
# Common kwargs for all learning algorithms
common_kwargs = {
...
for alg_name, alg_config in planning_pbar:
    ...
    for seed in seed_pbar:
        env_run = GridworldEnv(N=N, gamma=GAMMA, step_penalty=-0.1, terminal_reward=10.0)
        # Planning algorithms have different interface
...
print("  " + "=" * 68)
print(f"  {'Optimal (VI)':<25} {opt_return:>14.4f} {0.0:>8.4f} {vi_metrics.wall_time:>10.4f}")
print()
...
# Figure 1: Learning curves for all algorithms
fig1, ax1 = plt.subplots(figsize=(12, 7))

colors_learning = plt.cm.tab10(np.linspace(0, 1, len(learning_algorithms)))
colors_planning = plt.cm.Set2(np.linspace(0, 1, len(planning_algorithms)))

# Plot learning algorithms
for i, (alg_name, res) in enumerate(learning_algorithms.items()):
    if alg_name in all_results:
        data = all_results[alg_name]
        if len(data['checkpoint_episodes']) > 0:
            ax1.plot(data['checkpoint_episodes'], data['eval_mean'],
                    label=alg_name, color=colors_learning[i], linewidth=2)
            ax1.fill_between(data['checkpoint_episodes'],
                            data['eval_mean'] - data['eval_std'],
                            data['eval_mean'] + data['eval_std'],
                            color=colors_learning[i], alpha=0.15)

# Plot planning algorithms
for i, (alg_name, res) in enumerate(planning_algorithms.items()):
    if alg_name in all_results:
        data = all_results[alg_name]
        if len(data['checkpoint_episodes']) > 0:
            ax1.plot(data['checkpoint_episodes'], data['eval_mean'],
                    label=alg_name, color=colors_planning[i], linewidth=2, linestyle='--')
...
# Figure 2: Planning vs Learning comparison (early episodes)
fig2, axes = plt.subplots(1, 2, figsize=(14, 5))

# Left: Early learning comparison (first 5000 episodes)
ax2a = axes[0]
early_eps = 5000
for i, alg_name in enumerate(['Q-Learning', 'SARSA', 'Expected SARSA']):
    if alg_name in all_results:
        data = all_results[alg_name]
        eps = np.array(data['checkpoint_episodes'])
        mask = eps <= early_eps
        if np.any(mask):
            ax2a.plot(eps[mask], np.array(data['eval_mean'])[mask],
...
for i, alg_name in enumerate(['Rollout (heuristic)', 'MPC (H=5)', 'Truncated Rollout (m=5)']):
    if alg_name in all_results:
        ...
        if np.any(mask):
            ax2a.plot(eps[mask], np.array(data['eval_mean'])[mask],
                     label=alg_name, linewidth=2, linestyle='--')
...
ax2b = axes[1]
alg_names = list(sorted_algs)[:10]  # Top 10
returns = [all_results[name]['final_return_mean'] for name, _ in alg_names]
stds = [all_results[name]['final_return_std'] for name, _ in alg_names]
names = [name for name, _ in alg_names]
...
# Add optimal first
latex_table += f"Optimal (VI) & {opt_return:.4f} & -- & {vi_metrics.wall_time:.4f} \\\\\n"
latex_table += r"\midrule" + "\n"

# Learning algorithms
latex_table += r"\multicolumn{4}{l}{\textit{Model-Free RL}} \\" + "\n"
for alg_name in learning_algorithms.keys():
    if alg_name in all_results:
...
for alg_name in planning_algorithms.keys():
    if alg_name in all_results:
        res = all_results[alg_name]
        se = res['final_return_std'] / np.sqrt(NUM_SEEDS)
        latex_table += f"{alg_name} & {res['final_return_mean']:.4f} & {se:.4f} & {res['wall_time_mean']:.2f} \\\\\n"
...
for alg_name in ['Rollout (heuristic)', 'Rollout (VI-5)', 'MPC (H=5)', 'MPC (H=10)',
    ...
    if alg_name in all_results:
        res = all_results[alg_name]
        if len(res['eval_mean']) > 0:
            first_eval = res['eval_mean'][0]
...
print()
print("Key findings:")
print("  1. Planning methods (Rollout, MPC, Truncated Rollout) achieve near-optimal")
print("     performance from the first episode using model knowledge.")
print("  2. Model-free RL algorithms require thousands of episodes to learn,")
print("     but eventually converge to near-optimal policies.")
print("  3. Truncated rollout with VI warmup improves over pure heuristic rollout,")
print("     demonstrating Bertsekas's insight about VI stabilization.")
print("  4. MPC with longer horizons achieves better performance at computational cost.")
print()
