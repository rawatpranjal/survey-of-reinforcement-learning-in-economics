# The Running Example: a two-state machine-replacement MDP
...
def _search():
    ...
    at least min_off_modulus, the winner is the mildest: the largest logging
    keep-probability, which is the smallest departure from the target policy, and then
    the smallest feature ratio, which is the smallest asymmetry between the two states.
    Maximizing the modulus instead would just walk to whichever grid edge is furthest
    from the target policy and report an artifact of the grid.
    """
    cfg = SEARCH_CONFIG
    gamma = cfg["gamma"]
...
def generate_outputs(data):
    """One LaTeX table summarizing the running example, for Appendix A.2. No figure:
    ...
    with open(tex_path, "w") as f:
        ...
        f.write(
            ...
            "appendix. Rewards and transitions are the primitives; the optimal value, the "
            "stationary and discounted state distributions, and the projected-operator "
            "moduli are computed from them. The logging policy $\\mu$ keeps the machine "
        ...
        f.write(
            "\\hline\n\\multicolumn{3}{l}{"
            f"modulus of $\\Pi_{{d^{{\\pi^\\star}}}} T^{{\\pi^\\star}}$: "
            f"{proj['on_policy']['modulus']:.4f}; "
            f"of $\\Pi_{{d^\\mu}} T^{{\\pi^\\star}}$: {proj['off_policy']['modulus']:.4f}"
            "} \\\\\n"
        )
        f.write("\\hline\n\\end{tabular}\n\\end{table}\n")
    ...
    with open(td_path, "w") as f:
        f.write("\\begin{table}[h]\n\\centering\n")
        f.write(
            "\\caption{Tabular TD($0$) on the running example, evaluating $\\pi^\\star$ from "
...
def main():
    parser = argparse.ArgumentParser(description="Appendix A running example")
