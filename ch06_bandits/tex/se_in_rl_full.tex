...
Multi-armed bandit (MAB) problems represent a class of state-less reinforcement learning tasks where an agent repeatedly selects among different actions (arms) to maximize cumulative reward. Unlike regular reinforcement learning which are run in simulation, bandits are typically deployed in real-time which implies that sample efficiency and optimal exploration is critical. Integrating economic structure can improve bandit performance. 
...
\begin{equation}
U_t(i) = \alpha_i - \beta p_{i,t} + \gamma(r_t - p_{i,t}) + \epsilon_{i,t}
\end{equation}

where $r_t$ is the reference price evolving according to:

\begin{equation}
r_{t+1} = (1-\delta)r_t + \delta\frac{1}{n}\sum_i p_{i,t}
\end{equation}
