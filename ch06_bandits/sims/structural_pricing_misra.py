...
class UCB_PI:
    ...
    def _update_partial_identification(self, price: float, segment_id: int, purchased: bool):
        """Update segment valuation bounds based on observed purchase decision."""
        if purchased:
            self.segment_p_min[segment_id] = max(self.segment_p_min[segment_id], price)
        else:
            self.segment_p_max[segment_id] = min(self.segment_p_max[segment_id], price)

    def _estimate_delta(self) -> float:
        """Estimate δ from observed bounds (vectorized)."""
        valid = self.segment_p_max > self.segment_p_min
        if valid.sum() >= 2:
            delta_estimates = (self.segment_p_max[valid] - self.segment_p_min[valid]) / 2
            delta_max = delta_estimates.max()
            mean_delta = delta_estimates.mean()
            shrinkage = 0.5 * (delta_max - mean_delta)
            self.delta_hat = delta_max - shrinkage
        elif valid.sum() == 1:
            self.delta_hat = (self.segment_p_max[valid] - self.segment_p_min[valid])[0] / 2
    ...
    def _compute_profit_bounds(self) -> Tuple[np.ndarray, np.ndarray]:
        ...
        # Compute segment midpoints and bounds
        v_hat = (self.segment_p_max + self.segment_p_min) / 2  # (S,)
    ...
    def update(self, arm: int, reward: float, segment_id: int, purchased: bool):
        ...
        self._update_partial_identification(self.prices[arm], segment_id, purchased)
...
for name, label in [('oracle', 'Oracle'), ('ucb_pi_tuned', 'UCB-PI-tuned'),
                    ('ucb_pi', 'UCB-PI'), ('ucb1', 'UCB1'),
                    ('lte', 'LTE (5%)'), ('thompson', 'Thompson')]:
    frac = mean_arms[name][OPTIMAL_ARM] / T
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_profit.png',
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_regret.png',
            dpi=300, bbox_inches='tight')
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_arms.png',
            dpi=300, bbox_inches='tight')
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_bounds.png',
            dpi=300, bbox_inches='tight')
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_delta.png',
            dpi=300, bbox_inches='tight')
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_regret_scaling.png',
            dpi=300, bbox_inches='tight')
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_dominated_checkpoints.png',
            dpi=300, bbox_inches='tight')
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_price_histogram.png',
            dpi=300, bbox_inches='tight')
...
plt.tight_layout()
fig.savefig('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_convergence.png',
            dpi=300, bbox_inches='tight')
...
\end{tabular}
""" % (
    final_profits['oracle'], mean_arms['oracle'][OPTIMAL_ARM] / T,
    ...
    mean_arms['ucb_pi_tuned'][OPTIMAL_ARM] / T, dominated_final_mean['ucb_pi_tuned'],
    ...
    mean_arms['ucb_pi'][OPTIMAL_ARM] / T, dominated_final_mean['ucb_pi'],
    ...
    mean_arms['thompson'][OPTIMAL_ARM] / T,
)

with open('/Users/pranjal/Code/rl/ch06_bandits/sims/structural_pricing_misra_results.tex', 'w') as f:
    f.write(latex_table)
